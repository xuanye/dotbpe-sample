...
namespace Survey.Core
{
    ...
    {
        public static bool IsAuthed(this IMessage message)
        {
            var indentityField = message.Descriptor.FindFieldByNumber(2);
            if (indentityField == null)
            {
                return false;
            }
            object o = indentityField.Accessor.GetValue(message);
